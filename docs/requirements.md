...
| FR-2 | `--layer-height` is a number greater than 0. The default is 3.0, or the `layer_height` key of the config file (FR-30). A value of 0 or less stops the command with `Invalid value for --layer-height: must be > 0` and exit code 2. So does `nan` or `inf`, with `must be a finite number`. | `cli.py::process_model` | `test_process_model_validation`, `test_cli`, `test_settings` |
...
| FR-6 | Mark options: `--mark-size` (default none, which keeps the FR-21 rule; a number greater than 0), `--mark-tolerance` (default 10.0), `--mark-min-distance` (default 10.0), `--available-shapes` (comma-separated, default `circle,square,triangle,arrow`), `--mark-angle` (degrees, default 0.0), `--mark-color` (default none). The numbers must be finite. `--mark-size` must be greater than 0, and `--mark-tolerance` and `--mark-min-distance` must not be negative. Shape names are trimmed and empty items dropped. The angle is converted to radians. Each of these except `--mark-color` can also come from the config file (FR-30). | `cli.py::process_model` | `test_cli` |
| FR-30 | A setting comes from the command line first, then a TOML config file, then its default. The file is `--config PATH`, or `layerforge.toml` in the current directory if it exists. Keys: `layer_height`, and in the `[marks]` table `size`, `tolerance`, `min_distance`, `shapes` (a list) and `angle` (degrees). The options `--mark-color`, `--scale-factor`, `--target-height`, `--stl-file` and `--output-folder` are not settings. An unknown key, a wrong type, or a value that is not finite or out of range (FR-2, FR-6) stops the command before the mesh is read, with exit code 2 and `<file>: <key>: <reason>`. A bad file value fails even when an option overrides it. A bad option value names the option instead. | `settings.py::load_settings`, `cli.py::process_model` | `test_settings`, `test_cli` |
...
| G-25 (#117) | The scale and target conflict check tests truthiness (FR-3, FR-4). | `cli.py::process_model`, `if scale_factor and target_height`. | `--scale-factor nan` or `-1` with `--target-height 5` exits 1 with the conflict message, and `--scale-factor 0` exits 2 with a value message. The same pair gives two exit codes. |
| G-26 (#118) | A config file is read without a message (FR-30). | `settings.py::load_settings`. A `layerforge.toml` with `layer_height = 4.5` gave 2 slices instead of 3, exit 0, empty output. | A stray file in the working directory changes a run and nothing says so. |
| G-27 (#119) | A bad config file is reported after the `--stl-file` prompt (FR-30, NFR-5). | click prompts while it parses options, and `process_model` reads the settings later. | The person types a path, then learns the file is bad. |
