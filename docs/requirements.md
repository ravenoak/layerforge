...
| G-28 (#135) | Bad option values and the scale and target conflict are reported after the `--stl-file` prompt (NFR-5). | click prompts while it parses options, and `process_model` and `load_settings` check the values later. #119 moved only the config file check. | The person types a path, then learns an option is bad. |
