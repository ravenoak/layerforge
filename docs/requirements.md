...
| G-24 (#87) | Laser, material and mark numbers cannot be set. Mark size, the sampling seed, the label size and the stroke width are fixed in code, and only five mark options exist. | `cli.py`, `models/reference_marks/config.py`, `slice.py::_calculate_mark_size` | Workshops differ in kerf, sheet thickness and text limits. Target: TR-16. |
