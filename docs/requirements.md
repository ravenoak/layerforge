...
| FR-2 | `--layer-height` is a number greater than 0. The default is 3.0, or the `layer_height` key of the config file (FR-30). A value of 0 or less stops the command with `Invalid value for --layer-height: must be > 0` and exit code 2. So does `nan` or `inf`, with `must be a finite number`. | `cli.py::process_model` | `test_process_model_validation`, `test_cli` |
