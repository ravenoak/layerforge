...
| FR-30 | A setting comes from the command line first, then a TOML config file, then its default. The file is `--config PATH`, or `layerforge.toml` in the current directory if it exists. Keys: `layer_height`, and in the `[marks]` table `size`, `tolerance`, `min_distance`, `shapes` (a list) and `angle` (degrees). The options `--mark-color`, `--scale-factor`, `--target-height`, `--stl-file` and `--output-folder` are not settings. An unknown key, a wrong type, or a value that is not finite or out of range (FR-2, FR-6) stops the command with exit code 2 and `<file>: <key>: <reason>`. The file is checked before the `--stl-file` prompt and before the value checks of the other options, so a bad file is reported before an option conflict (FR-4) or a bad option value. An unknown option or a missing `--config` path is refused first, also with exit code 2. A bad file value fails even when an option overrides it. A bad option value names the option instead. | `cli.py::_check_config_file`, `settings.py::read_config_file`, `settings.py::load_settings` | `test_settings`, `test_cli` |
