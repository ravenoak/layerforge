...
The target adds a TOML config file (`--config`, or `layerforge.toml` in the current directory) so that laser, material and mark numbers such as the kerf, the minimum hole size and the number height can be set once and reused. It also adds `--units`, `--kerf`, `--mark-size`, `--number-height`, `--allow-unaligned`, `--cut-color` and `--engrave-color`. Settings come from the command line first, then the file, then the default. It changes the defaults and meaning of `--mark-min-distance` and `--mark-tolerance`, and removes `--mark-color`. See [Alignment requirements](alignment_requirements.md#settings).
