...
| FR-27 | Mark shapes, all unfilled and all closed outlines. Every shape is anchored at its centre, and its size is the diameter of the smallest circle around that centre that holds the outline. Circle: default red. Square: four corners on that circle, so the side is the size divided by √2, default blue. Triangle: isosceles with a 40° apex and all three corners on the circle, default green. Arrow: a closed polygon of seven vertices, with the tip and the two tail corners on the circle, default black. Angle 0 points along +x (the apex of the triangle, the tip of the arrow) and angles turn counter-clockwise. The color option replaces the default. Each shape gives its outline as a polygon (a circle as a polygon of 64 vertices just outside the true circle, for checks) and its rotational symmetry: circle unlimited, square 4, triangle 1, arrow 1. Angle turns the shape. Angles are in radians everywhere inside the package; only the `--mark-angle` option takes degrees. | `svg/drawing/strategies/*`, `domain/shapes/*` | `test_shape_contract`, `test_svg_output`, `test_arrow_drawing_strategy` |
...
| G-31 (#149) | An output folder that cannot be written, or an empty one, is not checked before the slicing (NFR-5). | `_check_output_folder` rejects only a folder that is a file, a dangling symlink, or inside one. | A folder that cannot be written gives a `PermissionError` traceback after the slicing (exit 1). For an empty folder text, `generate_file_name` returns `/slice_000.svg`. |
