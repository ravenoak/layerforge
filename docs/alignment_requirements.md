...
| ID | Requirement | Closes |
|---|---|---|
| TR-5 | A reference mark is a hole. Its footprint is a closed outline that lies wholly inside the piece. The centre is at least `--mark-min-distance` from every outline of the piece, holes included. The footprints of two holes are at least `marks.min_web_ratio` × layer height apart, and so is a footprint from every outline of the piece (the material between cuts). The default ratio is 0.5. No footprint overlaps the number. The footprint is checked, not only the centre. Status: built in #85 except the number clause, which comes with #75. The command chooses the point of a new mark with a disc that holds the outline of every available shape at any angle (radius: half the size, and for the circle a little more, see TR-7), and checks the exact footprint after the marks are chosen. | G-21 (#85, done except the number) |
| TR-6 | The default mark size is the larger of `marks.min_hole_ratio` × layer height and `marks.min_hole_kerf_factor` × `kerf`. The layer height is the sheet thickness. The defaults are 1 and 1.5, so with a 3 mm sheet and 0.3 mm kerf the size is 3 mm. `--mark-size` sets a size. A size below that minimum is a warning, not an error, because the person knows the machine. The size does not depend on the model's size. The default `--mark-min-distance` is the mark size. Status: built in #62 and #76. The size, the minimum distance and the tolerance are worked out once per run from the layer height, and a 10 mm cube gets marks with the default options. | G-6 (#62, done), G-19 (#76, done) |
| TR-7 | Every shape has a closed outline (a circle or a polygon), anchored at its centre. Its size is the diameter of the smallest circle around the anchor that holds the outline. Angle 0 points along +x and angles turn counter-clockwise. Angles are radians in the package and degrees on the command line. Each shape states its rotational symmetry: circle unlimited, square 4, triangle 1 (isosceles), arrow 1 (a closed arrow outline). Status: built in #84. The outline of a circle is a polygon of 64 vertices just outside the true circle, so a check never passes a circle that crosses an edge. The checks use this polygon and not the exact circle that the drawing cuts (decided in #157): it reaches 0.12% past the true radius (5.00603 for 5 at size 10, measured), which is 0.0018 at size 3 and far below a kerf. The proportions are fixed: the triangle has a 40° apex, and the arrow has a head 1.2 times and a shaft 0.5 times the radius wide. | G-20 (#84, done) |
...
| `--kerf` | New (#62). Default 0.3 mm (proposed), stated in `--units`. |
| `--mark-size` | New (#87). Sets the size. Without it the size follows the sheet thickness and the kerf (TR-6, built in #62). |
| `--mark-min-distance` | Default changes from 10 to the mark size (built in #62). It measures centre to outline. |
